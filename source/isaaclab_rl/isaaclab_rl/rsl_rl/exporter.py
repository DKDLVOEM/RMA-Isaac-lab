...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def export(self, path, filename):
        ...
        if self.is_recurrent:
            ...
            if hasattr(self.actor, "base_obs_dim") and hasattr(self.actor, "priv_obs_dim"):
                obs_dim = self.actor.base_obs_dim + self.actor.priv_obs_dim
            elif (
                hasattr(self.actor, "base_obs_dim")
                and hasattr(self.actor, "history_obs_dim")
                and hasattr(self.actor, "history_length")
            ):
                obs_dim = self.actor.base_obs_dim + self.actor.history_obs_dim * self.actor.history_length
            elif hasattr(self.actor, "in_features"):
                obs_dim = self.actor.in_features
            elif hasattr(self.actor, "action_mlp") and hasattr(self.actor.action_mlp[0], "in_features"):
                obs_dim = self.actor.action_mlp[0].in_features
            else:
